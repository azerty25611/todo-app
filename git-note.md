...
그 후에 아래 명령어 입력해서 확인:
git --version
결과 예시: git version 2.44.0.windows.1
이렇게 나오면 이제 제대로 설치되고 연결된 것임.
(Git 설치는 했는데도 git 명령어 인식이 안 된다면, 환경변수 설정에 문제가 있을 수 있음.)


# 그래도 명령어를 못 알아듣고 Please tell me who you are와 같은 문구가 나올 경우
